...
# 115000 부터 돌릴것. 그 윗 번호는 ver.1 사용.
for i in tqdm(range(115000,60000,-1)):
    # 혹시 모를 try except
    try:
        ...
        if "video" in page.text:
            ...
            video = videolst
        # 하나의 리스트로 통합
        tmp_lst.append([title, img, video, text, i])
    except:
        print(i, " 오류")
    if i % 5000 ==1:
        # 5천개씩 csv 파일로 저장.
        pd.DataFrame(tmp_lst, columns= ['title','img','video','text','number']).to_csv(f"./jjalbox/jjalbox_N_{i//5000}.csv",index=False) #숫자가... 내림차순임..ㅎ....ㅈㅅ
        tmp_lst = [] # tmp_lst 초기화
