...
print('Before word filtering :')
print(df.info())

df_notna = df[df.notna().all(axis=1)]
df_isna = df[df['hashtag'].isnull()]
...
df_filtered = pd.concat([filter(df_notna), df_isna])
df_filtered.to_csv(r'C:\Memoticon\data\004_word_filtering.csv')
print(f'After word filtering :')
